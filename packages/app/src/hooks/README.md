...
```typescript
// 方式1: 直接导入
import { useAccess } from '@jinghe-sanjiaoroad-app/app/hooks'
const { hasAccessByRoles, hasAccessByCodes } = useAccess()

// 方式2: 命名空间导入
import { access } from '@jinghe-sanjiaoroad-app/app/hooks'
...
**使用示例：**
```typescript
import { dict } from '@jinghe-sanjiaoroad-app/app/hooks'
...
```typescript
// 方式1: 直接导入
import { useRequest } from '@jinghe-sanjiaoroad-app/app/hooks'
...
// 方式2: 命名空间导入
import { request } from '@jinghe-sanjiaoroad-app/app/hooks'
...
```typescript
// 方式1: 直接导入
import { useScroll } from '@jinghe-sanjiaoroad-app/app/hooks'
...
// 方式2: 命名空间导入
import { scroll } from '@jinghe-sanjiaoroad-app/app/hooks'
...
```typescript
// 方式1: 直接导入
import { useUpload } from '@jinghe-sanjiaoroad-app/app/hooks'
const { 
  loading, 
  error, 
  progress, 
...
// 方式2: 命名空间导入
import { upload } from '@jinghe-sanjiaoroad-app/app/hooks'
...
import { 
  ...
  useUpload, 
  dict 
} from '@jinghe-sanjiaoroad-app/app/hooks'
```
