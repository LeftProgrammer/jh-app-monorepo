...
JH App 框架的 Hooks 模块，提供常用的 Vue 3 组合函数，包括权限控制、字典处理、请求管理、滚动加载和文件上传等功能。
...
```typescript
// 方式1: 直接导入
import { useAccess } from '@jh-app/app/hooks'
const { hasAccessByRoles, hasAccessByCodes } = useAccess()

// 方式2: 命名空间导入
import { access } from '@jh-app/app/hooks'
...
**使用示例：**
```typescript
import { dict } from '@jh-app/app/hooks'
...
```typescript
// 方式1: 直接导入
import { useRequest } from '@jh-app/app/hooks'
...
// 方式2: 命名空间导入
import { request } from '@jh-app/app/hooks'
...
```typescript
// 方式1: 直接导入
import { useScroll } from '@jh-app/app/hooks'
...
// 方式2: 命名空间导入
import { scroll } from '@jh-app/app/hooks'
...
```typescript
// 方式1: 直接导入
import { useUpload } from '@jh-app/app/hooks'
const { 
  loading, 
  error, 
  progress, 
...
// 方式2: 命名空间导入
import { upload } from '@jh-app/app/hooks'
