...
const props = withDefaults(defineProps<{
  ...
  detailUrl: '/pages-bpm/processInstance/detail/index?id={id}&type=copy',
})
...
async function getList() {
  ...
  try {
    const data = await getProcessInstanceCopyPage({
      ...queryParams.value,
      processInstanceName: searchName.value || undefined,
    })
...
function handleDetail(item: ProcessInstanceCopy) {
  const url = props.detailUrl.replace('{id}', item.processInstanceId)
  uni.navigateTo({ url })
}
