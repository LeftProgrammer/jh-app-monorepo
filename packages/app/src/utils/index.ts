 ...
 * 获取 API 基础地址
 *
 * @description 从框架配置中获取 baseUrl，这是通过 initFramework() 配置的基础地址
 *              如果需要根据微信小程序环境动态获取地址，请在项目级实现 getEnvBaseUrl()
 *
 * @example
 * // 框架包内部或简单场景使用
 ...
 * 获取基础路径的根路径
 *
 * @description 从 baseUrl 中提取根路径，如 http://localhost:48080
 *              用于 websocket 等需要根路径的场景
 *
 * @example
 * // getBaseUrl() 返回 'http://localhost:48080/admin-api'
 * // getBaseUrlRoot() 返回 'http://localhost:48080'
 */
export function getBaseUrlRoot() {
  const baseUrl = getBaseUrl()
  if (!baseUrl) {
    return ''
  }
...
export function redirectAfterLogin(redirectUrl?: string) {
  const homePage = _getConfiguredHomePage()
