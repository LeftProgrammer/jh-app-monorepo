<template>
	<wd-upload v-bind="restProps" :action="uploadAction" v-model:file-list="resolvedFileList"
		:before-preview="beforePreview" @change="handleUploadChange" @remove="emit('remove', $event)"
		@success="emit('success', $event)" @fail="emit('fail', $event)" @progress="emit('progress', $event)"
		@oversize="emit('oversize', $event)" @chooseerror="emit('chooseerror', $event)"
		...
		async (newIds) => {
			...
			try {
				...
				if (Array.isArray(newIds) && newIds.length > 0) {
					...
					if (typeof firstItem === 'object' && firstItem !== null && ('id' in firstItem || 'response' in firstItem)) {
				...
				if (typeof newIds === 'number') {
					...
					idList = newIds.split(',').map(id => id.trim()).filter(Boolean)
			...
			uni.downloadFile({
				...
				success: (res) => {
					if (res.statusCode === 200 && res.tempFilePath) {
						...
						console.warn('Download failed with status:', res.statusCode)
						resolve(false)
					}
				},
				fail: (err) => {
