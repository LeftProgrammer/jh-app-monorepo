...
const props = defineProps<{
  fileId?: string | number | (string | number)[] | UploadFileItem[]
  action?: string
  /** 是否返回完整文件对象列表（默认返回逗号分隔的 id 字符串） */
  returnObject?: boolean
}>()
const emit = defineEmits<{
  (e: 'update:fileId', value: string | (string | number | UploadFileItem)[]): void
...
function handleUploadChange(e: UploadChangeEvent) {
  ...
  emit('update:fileId', props.returnObject ? (standardizedList as any) : ids.join(','))
    ...
    try {
      ...
      if (Array.isArray(newIds) && newIds.length > 0) {
        ...
        if (
          ...
          const fileItems = newIds as UploadFileItem[]
          resolvedFileList.value = fileItems.map(file => {
            ...
            return {
              ...file,
              uid: file.uid ?? (Number(fileId) || Math.random()), // 必须有 uid
              name: file.name || 'unknown',
              url: file.url || '',
              status: file.status || 'success',
