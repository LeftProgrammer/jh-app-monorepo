 ...
 *
 * @note 持久化插件使用：
 *       - 基础使用：import { store } from '@jinghe-sanjiaoroad-app/framework/store'
 *       - 自定义配置：import { createAppStore } from '@jinghe-sanjiaoroad-app/framework/store'
 *       - 主要方法：import { createPersistedState } from '@jinghe-sanjiaoroad-app/framework/store'
 *       - 完整访问：import { persistPlugin } from '@jinghe-sanjiaoroad-app/framework/store'
 *
 * @note 包体积优化：
...
// 注意：不在包内部激活 Pinia 实例，由外部项目决定何时激活
// 这样可以避免重复激活导致的问题
 ...
 * ```typescript
 * // 方式1: 使用默认配置（推荐）
 * import { store } from '@jinghe-sanjiaoroad-app/framework/store'
 * app.use(store)
 *
 * // 方式2: 使用自定义配置
 * import { createAppStore } from '@jinghe-sanjiaoroad-app/framework/store'
 ...
 * import { createPersistedState } from '@jinghe-sanjiaoroad-app/framework/store'  // 按需导入
 ...
 *
 * // 方式4: 使用插件的其他方法（按需导入）
 * import { persistPlugin } from '@jinghe-sanjiaoroad-app/framework/store'
 * // 假设插件有其他方法
