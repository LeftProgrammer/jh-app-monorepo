  ...
  >
    <view class="h-full flex flex-col p-32rpx">
      ...
      <view class="mb-32rpx flex items-center justify-between">
        <view class="text-36rpx text-[#333] font-semibold"> 消息详情 </view>
      ...
      <view v-if="formData" class="flex flex-1 flex-col overflow-hidden space-y-24rpx">
        <view class="flex items-start">
          ...
          <text class="text-28rpx text-[#333]">{{ formData.senderName }}</text>
        </view>
        <view class="flex items-start">
        ...
        <view class="flex items-start">
          ...
          <wd-tag v-if="formData.readStatus" type="success" plain> 已读 </wd-tag>
          <wd-tag v-else type="warning" plain> 未读 </wd-tag>
        </view>
