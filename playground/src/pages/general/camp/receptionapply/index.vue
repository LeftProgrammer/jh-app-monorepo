  ...
  <view class="yd-page-container">
    ...
    >
      ...
      >
        ...
        <view class="flex justify-between items-center mb-19rpx text-27rpx gap-31rpx">
          <text class="text-#4E5969">接待对象</text>
          <text class="flex-1 text-ellipsis text-right text-#1D2129">
            {{ item.reception }}
          </text>
        </view>
        <view class="flex justify-between items-center text-27rpx gap-31rpx">
          <text class="text-#4E5969">用餐时间</text>
          <text class="flex-1 text-ellipsis text-right text-#1D2129">
            {{ formatDateTimeHour(item.startMealTime) }}~{{
              formatDateTimeHour(item.endMealTime)
            }}
          </text>
        </view>
      </view>
...
import { getPage, deleteById } from "@/api/general/camp/receptionapply";
...
function routerTo(type: string, item?: any) {
  uni.navigateTo({
    url: `/pages/general/camp/receptionapply/ReceptionApplyForm?type=${type}&id=${
      item?.id || ""
    }`
  });
}
