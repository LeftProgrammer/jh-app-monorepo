    ...
    >
      ...
      <wd-cell-group border title="审批信息">
        ...
        <UserPicker
          ...
          v-if="
            todoTask?.name == '拟办' ||
            todoTask?.name == '分管领导审批' ||
            todoTask?.name == '文书处理' ||
            todoTask?.name == '相关领导/部门/工区阅处' ||
            formData?.status == 2
          "
        />
        <UserPicker
...
const safeFileList = (v: any): UploadItem[] => (Array.isArray(v) ? v : []);

const normalize01 = (v: any) => (v === 1 || v === "1" || v === true ? 1 : 0);

/** wot upload change：把 response 里的 id 填到每一项上 */
function fileChange(fileList: UploadItem[], field: "file" | "otherFile" | "sealFile") {
  const list = (fileList || []).map((x: any) => {
    // 新上传的才有 response；回显的文件一般没 response
    if (x?.response) {
      try {
        x.id = JSON.parse(x.response)?.data?.id;
      } catch {
        // 有的项目 response 已经是对象
        x.id = x.response?.data?.id;
      }
    }
    return x;
  });
  (formData.value as any)[field] = list;
}
...
const formData = ref<any>({
  ...
  user1: "",
  user2: userInfo.value.id,
  user3: "",
  user4: userInfo.value.id,
...
const formRules = {
  ...
  user4: [{ required: true, message: "文书处理不能为空" }],
  user5: [{ required: true, message: "相关领导/部门/工区阅处不能为空" }]
};
const menuType = ref("");
onLoad((options) => {
...
async function handleSubmit(status?: number) {
  ...
  try {
    const data: any = {
      ...
      sendDate: new Date(formData.value.sendDate).getTime(),
      sendUnit: userInfo.value.department,
      menuType: "1",
      status
    };
...
onMounted(() => {
  if (props.id) getInfo();
  else formData.value.user2 = userInfo.value.id;
});
</script>
