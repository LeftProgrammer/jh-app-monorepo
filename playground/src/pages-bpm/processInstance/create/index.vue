<template>
  ...
  <view class="yd-page-container">
    ...
    <!-- 分类标签 -->
    <wd-tabs v-model="activeCategory" slidable="always" sticky @click="handleTabClick">
      <wd-tab
    ...
    >
      ...
      >
        ...
        <!-- 流程列表 -->
        <view
          v-if="groupedDefinitions[item.code]?.length"
          class="overflow-hidden rounded-16rpx bg-white"
        >
          <view
...
function updateCategoryPositions() {
  ...
  query.exec((res) => {
    if (res && res[0]) {
      ...
      res[0].forEach(
        (item: { top: number; dataset?: { category?: string } }, index: number) => {
