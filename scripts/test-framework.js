...
// 检查一体化包文件
const jhAppDir = path.resolve(__dirname, '../packages/jh-app/src');
const jhAppFiles = [
  'index.ts',
  'http/index.ts',
  'store/index.ts',
...
// 检查组件文件
const componentsDir = path.resolve(__dirname, '../packages/jh-app/src/components');
...
// 检查工具文件
const utilsDir = path.resolve(__dirname, '../packages/jh-app/src/utils');
...
console.log('  3. 运行 pnpm --filter @jh-app/app dev 开发一体化包');
console.log('  4. 运行 pnpm --filter @jh-app/app build 构建一体化包');
console.log('  5. 打开 examples/simple/index.html 测试基础功能');
console.log('\n🎯 一体化包优势:');
console.log('  ✅ 只需安装 @jh-app/app 一个依赖');
